...
backend masters_api
  mode tcp
  balance source
  server boostrap-server.${FQDN}:6443 check
  server master-1.${FQDN}:6443 check
  server master-2.${FQDN}:6443 check
  server master-3.${FQDN}:6443 check

backend masters_machine_config
  mode tcp
  balance source
  server boostrap-server.${FQDN}:22623 check
  server master-1.${FQDN}:22623 check
  server master-2.${FQDN}:22623 check
  server master-3.${FQDN}:22623 check

backend router_http
  mode tcp
  server worker-1.${FQDN}:80 check
  server worker-2.${FQDN}:80 check

backend router_https
  mode tcp
  server worker-1.${FQDN}:443 check
